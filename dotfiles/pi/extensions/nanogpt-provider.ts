 ...
 *
 * Registers nano-gpt.com (https://nano-gpt.com/api/v1) as an
 * OpenAI-compatible provider in pi.
 *
 * Auth: reads NANOGPT_API_KEY env var (set system-wide via sops-nix
 *       sessionVariables; secret: nanogpt_api_key).
 *
 * Autodetection:
 *   - Fetches GET /models?detailed=true at startup and derives model
 *     fields from the response (context_length, max_output_tokens,
 *     capabilities.{vision,reasoning}, pricing.{prompt,completion}).
 *   - Missing fields fall back to id-based family inference (same map
 *     as cheapcompute). API-provided fields always win over inference.
 *
 * Usage: /model nanogpt/<model-id>
 */

import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";

const BASE_URL = "https://nano-gpt.com/api/v1";
const API_KEY_ENV = "NANOGPT_API_KEY";
...
// ---------------------------------------------------------------------------
// API-field extraction (NanoGPT detailed + OpenRouter-style + generic aliases)
// ---------------------------------------------------------------------------
...
function apiMaxTokens(model: RawModel): number | undefined {
  const value = pickNum(
    model,
    ["max_output_tokens", "max_tokens", "max_completion_tokens", "output_tokens"],
    0,
  );
  return value > 0 ? Math.floor(value) : undefined;
}

/** Reasoning flag from NanoGPT capabilities / top-level / :thinking suffix. */
function apiReasoning(model: RawModel): boolean | undefined {
  const caps = model.capabilities;
  if (caps && typeof caps === "object") {
    const c = caps as Record<string, unknown>;
    if (typeof c.reasoning === "boolean") return c.reasoning;
  }
  for (const key of ["reasoning", "supports_reasoning", "reasoning_capable", "thinking", "supports_thinking"]) {
    const value = model[key];
    if (typeof value === "boolean") return value;
  }
  if (model.id.toLowerCase().includes(":thinking")) return true;
  return undefined;
}

/** Input modalities from NanoGPT capabilities / modality aliases. */
function apiInput(model: RawModel): ("text" | "image")[] | undefined {
  const caps = model.capabilities;
  if (caps && typeof caps === "object") {
    const c = caps as Record<string, unknown>;
    if (typeof c.vision === "boolean") return c.vision ? ["text", "image"] : ["text"];
  }
...
/** Cost per 1M tokens from API response (NanoGPT/OpenRouter pricing or flat keys). */
...
async function fetchModels(apiKey: string): Promise<RawModel[]> {
  ...
  return payload.data.filter(
    (m): m is RawModel => !!m && typeof m === "object" && typeof (m as RawModel).id === "string",
  );
}
...
export default async function (pi: ExtensionAPI) {
  ...
  if (process.env[API_KEY_ENV]) {
    try {
      ...
      console.error(
        `[nanogpt-provider] model discovery failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  } else {
    console.error(
      `[nanogpt-provider] ${API_KEY_ENV} not set — provider registered without models. ` +
        `Set the env var (sops secret: nanogpt_api_key) and /reload.`,
    );
  }
